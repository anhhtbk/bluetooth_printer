...
import 'package:bluetooth_printer/receipt/receipt_section_text.dart';
import 'package:bluetooth_printer/scanner/blue_scanner.dart';
...
import 'models/blue_device.dart';
...
class BluePrintPos {
  ...
  Future<void> printText(List<int> bytes) async {
    _printProcess(bytes);
  }
  ...
  Future<void> _printProcess(List<int> byteBuffer) async {
    try {
      if (selectedDevice == null) {
        debugPrint('$runtimeType - Device not selected');
  ...
  Future<Uint8List> _getQRImage(String text, double size) async {
    try {
      ...
      debugPrint('$runtimeType - $exception');
      rethrow;
    }
  }
  ...
  static Future<Uint8List> contentToImage({
    ...
    try {
      results = await WebcontentConverter.contentToImage(
          content: content, duration: Platform.isIOS ? 2000 : duration);
